...
func createScheduledChatMessageToSchedulerStream() (*jetstream.PubAck, error) {
	...
	scheduledAt := currentTime.Add(5 * time.Second)
	log.Println(scheduledAt.Format(time.RFC3339), "에 발행 예약!")
	...
	pubAck, err := JS.PublishMsg(context.Background(), &nats.Msg{
		Header: nats.Header{
			...
			"Nats-Schedule-TTL":    []string{"10s"},            // TTL 설정 (즉 실제 target에 도달 후 보관을 하고 있을 기간을 의미**)
