...
* Nats-Schedule-TTL : 예약 시각에 발행 후에 Nats-Schedule-Target subject로 발행된 메세지를 Stream에 얼마나 오래 보관할 것인지를 결정합니다.
  단위는 m(분), s(초) 중 선택해서 설정 가능합니다. 0으로 설정 불가합니다.
...
- stream에서 메세지 발행되는 경우 표출되는 주요 헤더 설명(이외 몇개의 헤더들이 더 있을 수 있으나 중요한 헤더만 설명을 기재했습니다)

```
* Nats-Schedule-Next:
다음 예약 메세지를 다시 발행할지 여부.
Nats-Schedule-Header에 @at으로 설정하는 경우 단 1번만 메세지 발행되기 때문에 이 값은 purge로 표출됩니다.
그리고 purge 라는 의미는 nats-schedule-ttl 에 설정된 기간 만큼만 nats-schedule-target에 지정한 subject 메세지를 보관하다가 삭제한단 의미입니다.

* Nats-Scheduler :
예약 메세지 발행 시 사용했던 subject입니다. 예) schedule.pending.{scheduleId}

* Nats-Stream :
해당 메세지를 발행한 stream의 이름입니다

* Nats-Subject :
현재 stream에서 발행해준 메세지의 subject입니다. 즉, nats-schedule-target과 같은 값입니다.

* Nats-TTL:
nats-schedule-ttl과 같은 값입니다.

* Nats-Time-Stamp:
nats-subject 헤더의 값을 가지는 subject 메세지가 발행된 시각입니다.

++ 이외에 내가 커스텀하게 추가한 헤더들을 확인할 수 있습니다.
```
